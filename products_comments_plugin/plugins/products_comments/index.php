...
if($action=="product_details"){
   ...
   if($sec_img->verify_string($sec_string)){
            ...
            db_query("insert into store_products_comments (userid,content,cat,active,date) values('$userid','$content','$id','1',now())");
...
$qr = db_query("select * from store_products_comments where cat ='$id' and active=1");
 ...
 $dx = db_qr_fetch("select * from ".members_table_replace('store_clients')." where ".members_fields_replace('id')."='$data[userid]'",MEMBER_SQL);
          ...
          if($is_admin){
          print " &nbsp;[<a href='".iif($admin_folder,$admin_folder,"admin")."/index.php?action=products_comment_del&id=$data[id]&cat=$id'>Õ–›</a>]";
              }
