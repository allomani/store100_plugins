...
$count = db_qr_fetch("select count(*) as count from songs_dedications where active=0");
         print "<br>";     
print_admin_table("<b>«Âœ«∆«   ‰ Ÿ— «·„Ê«›ﬁ… : </b> <a href='index.php?action=dedications'>".intval($count['count'])." </a>");    
...
//---------- edit -------------------
 ...
 db_query("update songs_dedications set user='".db_clean_string($user)."',msg='".db_clean_string($msg)."' where id='$id'");
         }
...
while($data = db_fetch($qr)){
  ...
  ".iif($data['active'],"<a href='index.php?action=dedications_disable&id=$data[id]'> ⁄ÿÌ·</a>","<a href='index.php?action=dedications_enable&id=$data[id]'> ›⁄Ì·</a>")." - 
